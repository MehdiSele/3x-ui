...
install_firewall() {
    ...
    if ufw status | grep -q "Status: active"; then
        ...
        ufw allow http
        ufw allow https
        ufw allow 9999/tcp #webPort
        ufw allow 8888/tcp #subport
...
open_ports() {
    ...
    read -rp "Enter the ports you want to open (e.g. 80,443,9999 or range 400-500): " ports

    # Check if the input is valid
    if ! [[ $ports =~ ^([0-9]+|[0-9]+-[0-9]+)(,([0-9]+|[0-9]+-[0-9]+))*$ ]]; then
        echo "Error: Invalid input. Please enter a comma-separated list of ports or a range of ports (e.g. 80,443,9999 or 400-500)." >&2
        exit 1
    fi
...
delete_ports() {
    ...
    if [[ $choice -eq 1 ]]; then
        ...
        # Deleting by ports
        read -rp "Enter the ports you want to delete (e.g. 80,443,9999 or range 400-500): " ports
